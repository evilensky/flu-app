...
set :application, 'flu-app'
set :repo_url, "git@github.com:evilensky/#{ fetch(:application) }.git"
set :rvm_type, :system
...
#TODO production should be converted to /var/www/apps as well.
...
<VirtualHost *:80>
  ServerName fluomics-staging.cbits.northwestern.edu
  Redirect permanent / https://fluomics-staging.cbits.northwestern.edu/
</VirtualHost>
...
#  SSLCertificateKeyFile /etc/pki/tls/private/cbits-railsapps.nubic.northwestern.edu.key
  ...
  RailsBaseURI /
  PassengerDebugLogFile /var/log/httpd/#{ fetch(:deploy_to) }passenger.log
...
<VirtualHost *:80>
  ServerName flumonics.northwestern.edu
  Redirect permanent / https://fluomics.northwestern.edu/
</VirtualHost>
...
<VirtualHost *:80>
  ServerName fluomics
  Redirect permanent / https://fluomics
</VirtualHost>
...
#  SSLCertificateKeyFile /etc/pki/tls/private/cbits-railsapps.nubic.northwestern.edu.key
  ...
  <Directory #{ fetch(:deploy_to) }/current/public >
    ...
    Options -MultiViews
      ...
      #TODO We should probably figure out something else than echo, what if we have to quote inside the paragraph?
